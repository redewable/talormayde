...
export default function Admin() {
  ...
  if (loading) return <div className="min-h-screen bg-black text-white flex items-center justify-center font-mono">:: VERIFYING ::</div>;

  return (
    <div className="min-h-screen bg-zinc-950 text-white p-6 md:p-12 pb-32 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 border-b border-white/5 pb-6 gap-6">
            ...
            <p className="text-zinc-500 text-sm font-mono mt-1 uppercase tracking-widest">Director: {user?.email}</p>
            </div>
            <div className="flex gap-4">
            <Link href="/"><button className="p-3 bg-white/5 rounded-full text-zinc-400 hover:bg-white/10 hover:text-white transition-colors"><LogOut size={18} className="rotate-180" /></button></Link>
        ...
        {/* TABS */}
        <div className="flex gap-8 mb-8 border-b border-white/5 overflow-x-auto">
            {[{id: "projects", label: "Work", icon: Briefcase}, {id: "clients", label: "Clients", icon: Users}, {id: "inbox", label: "Inquiries", icon: Inbox}, {id: "blog", label: "Journal", icon: FileText}].map(t => (
                <button key={t.id} onClick={() => { setTab(t.id as any); setSelectedClient(null); }} className={`pb-4 text-xs font-mono font-bold tracking-widest uppercase border-b-2 whitespace-nowrap flex items-center gap-2 ${tab === t.id ? "border-white text-white" : "border-transparent text-zinc-600 hover:text-zinc-400"}`}>
            ...
            <>
                <div className="flex justify-between items-center mb-8">
                    <div className="flex bg-white/5 rounded-lg p-1 border border-white/5">
                        <button onClick={() => setViewMode("grid")} className={`p-2 rounded-md transition-colors ${viewMode === "grid" ? "bg-white/10 text-white" : "text-zinc-500 hover:text-white"}`}><LayoutGrid size={16} /></button>
                        <button onClick={() => setViewMode("list")} className={`p-2 rounded-md transition-colors ${viewMode === "list" ? "bg-white/10 text-white" : "text-zinc-500 hover:text-white"}`}><List size={16} /></button>
                    </div>
                ...
                <div className="min-h-[400px]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {projects.map((p) => (
                        <div key={p.id} className="group relative bg-black/20 border border-white/5 rounded-2xl overflow-hidden hover:border-white/20 transition-all">
                            <div className="h-48 bg-black w-full relative">
                                ...
                                <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => openEdit(p, "project")} className="p-2 bg-black/50 rounded-lg hover:bg-white hover:text-black transition-colors"><Edit2 size={14} /></button>
                                    <button onClick={() => handleDelete("projects", p.id)} className="p-2 bg-red-900/50 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition-colors"><Trash2 size={14} /></button>
                                </div>
                            </div>
                            <div className="p-5">
                    ...
                    <div className="bg-black/20 border border-white/5 rounded-2xl overflow-hidden">
                        ...
                        <thead className="bg-white/5 text-zinc-500 font-mono uppercase text-xs"><tr><th className="p-4">Order</th><th className="p-4">Project</th><th className="p-4 hidden md:table-cell">Details</th><th className="p-4 text-right">Actions</th></tr></thead>
                        <tbody className="divide-y divide-white/5">
                            {projects.map((p, index) => (
                            <tr key={p.id} className="hover:bg-white/5 transition-colors">
                                <td className="p-4 flex gap-2 items-center"><div className="flex flex-col gap-1"><button onClick={() => moveProject(index, 'up')} className="hover:text-white"><ArrowUp size={12}/></button><button onClick={() => moveProject(index, 'down')} className="hover:text-white"><ArrowDown size={12}/></button></div><span className="font-mono text-zinc-500 text-xs ml-2">#{p.order}</span></td>
                                <td className="p-4"><div className="flex items-center gap-4">{p.imageUrl ? <img src={p.imageUrl} className="w-10 h-10 rounded-lg object-cover border border-white/10" /> : <div className="w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center"><ImageIcon size={14}/></div>}<p className="font-bold text-white text-base">{p.title}</p></div></td>
                                ...
                                <td className="p-4 text-right space-x-4"><button onClick={() => openEdit(p, "project")} className="text-zinc-500 hover:text-white transition-colors text-xs uppercase tracking-widest font-bold">Edit</button><button onClick={() => handleDelete("projects", p.id)} className="text-zinc-500 hover:text-red-500 transition-colors text-xs uppercase tracking-widest font-bold">Delete</button></td>
                            </tr>
                            ))}
                        </tbody>
                        </table>
            ...
            <div className="max-w-6xl mx-auto">
                {!selectedClient ? (
                    <>
                        ...
                        <div className="grid gap-4">{clients.map((client) => (<div key={client.id} onClick={() => setSelectedClient(client)} className="bg-black/20 border border-white/5 p-6 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-white/20 transition-all cursor-pointer"><div className="flex items-center gap-4 w-full md:w-auto"><div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center text-white font-mono text-xs border border-white/10">{client.progress}%</div><div><h3 className="text-xl font-light text-white group-hover:text-zinc-300 transition-colors">{client.projectName}</h3><p className="text-zinc-500 font-mono text-xs tracking-widest uppercase">{client.email}</p></div></div><div className="flex-grow w-full md:px-12"><div className="flex justify-between text-[10px] text-zinc-500 mb-2 uppercase tracking-widest font-mono"><span>{client.status}</span><span>Due: {client.dueDate}</span></div><div className="w-full h-1 bg-zinc-800 rounded-full overflow-hidden"><div className="h-full bg-white" style={{ width: `${client.progress}%` }} /></div></div><div className="text-zinc-500 text-[10px] uppercase font-bold tracking-widest group-hover:text-white">Open File &rarr;</div></div>))}</div>
                    </>
                ) : (
                    <div className="min-h-[500px]">
                        {/* Detail view logic is same as previous, just rendering checks */}
                        <div className="flex items-center gap-6 mb-8 border-b border-white/5 pb-8"><button onClick={() => setSelectedClient(null)} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition-colors"><ArrowUp size={18} className="-rotate-90" /></button><div><h2 className="text-3xl font-light">{selectedClient.projectName}</h2><p className="text-zinc-500 font-mono text-xs tracking-widest uppercase">{selectedClient.email}</p></div><div className="ml-auto flex gap-3"><button onClick={() => handleDelete("clients", selectedClient.id)} className="px-4 py-2 border border-red-900/30 text-red-500/50 rounded-lg hover:bg-red-900/10 hover:text-red-500 transition-all font-mono text-[10px] uppercase tracking-widest">Terminate Project</button></div></div>
                        <div className="grid grid-cols-3 gap-6 mb-8"><div className="p-6 bg-black/20 border border-white/5 rounded-2xl"><label className="text-[10px] text-zinc-500 uppercase font-mono tracking-widest mb-2 block">Status Phase</label><input className="w-full bg-transparent border-b border-zinc-800 focus:border-white outline-none pb-2 text-lg font-light" value={selectedClient.status} onChange={(e) => updateClientField("status", e.target.value)} /></div><div className="p-6 bg-black/20 border border-white/5 rounded-2xl"><label className="text-[10px] text-zinc-500 uppercase font-mono tracking-widest mb-2 block">Progress ({selectedClient.progress}%)</label><input type="range" min="0" max="100" className="w-full accent-white" value={selectedClient.progress} onChange={(e) => updateClientField("progress", Number(e.target.value))} /></div><div className="p-6 bg-black/20 border border-white/5 rounded-2xl"><label className="text-[10px] text-zinc-500 uppercase font-mono tracking-widest mb-2 block">Next Milestone</label><input className="w-full bg-transparent border-b border-zinc-800 focus:border-white outline-none pb-2 text-lg font-light" value={selectedClient.nextMilestone} onChange={(e) => updateClientField("nextMilestone", e.target.value)} /></div></div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[600px]"><div className="bg-black/20 border border-white/5 rounded-2xl p-6 flex flex-col h-full"><h3 className="text-xs font-mono font-bold text-zinc-500 uppercase tracking-widest mb-4 flex items-center gap-2"><MessageSquare size={14}/> Correspondence Log</h3><div className="flex-grow overflow-y-auto space-y-4 mb-4 pr-2 custom-scrollbar">{selectedClient.messages?.length === 0 && <p className="text-zinc-700 text-center py-20 italic font-serif">Begin the conversation.</p>}{selectedClient.messages?.map((msg: any, i: number) => (<div key={i} className={`flex flex-col ${msg.sender === 'admin' ? 'items-end' : 'items-start'}`}><div className={`p-4 rounded-2xl max-w-[85%] text-sm leading-relaxed ${msg.sender === 'admin' ? 'bg-white/10 text-white border border-white/10' : 'bg-black/40 text-zinc-300'}`}>{msg.text}</div><span className="text-[10px] text-zinc-600 mt-2 uppercase tracking-widest">{msg.sender === 'admin' ? 'Director' : 'Client'} • {new Date(msg.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span></div>))}<div ref={scrollRef} /></div><div className="flex gap-2"><input className="flex-grow bg-transparent border-b border-zinc-700 py-3 text-white outline-none focus:border-white transition-colors font-light" placeholder="Type message..." value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} /><button onClick={sendMessage} className="p-3 bg-white text-black rounded-lg hover:bg-zinc-200 transition-colors"><Send size={18} /></button></div></div><div className="bg-black/20 border border-white/5 rounded-2xl p-6 flex flex-col h-full overflow-y-auto"><div className="mb-8 space-y-6"><div><h3 className="text-xs font-mono font-bold text-zinc-500 uppercase tracking-widest mb-2 flex items-center gap-2"><Globe size={14}/> Staging URL</h3><div className="flex gap-2"><input className="flex-grow bg-transparent border-b border-zinc-800 py-2 text-zinc-300 text-sm font-light outline-none focus:border-white" value={selectedClient.previewUrl || ""} onChange={(e) => updateClientField("previewUrl", e.target.value)} placeholder="https://staging.talormayde.com" />{selectedClient.previewUrl && <a href={selectedClient.previewUrl} target="_blank" className="p-2 bg-white/5 rounded hover:bg-white hover:text-black transition-colors"><ExternalLink size={16} /></a>}</div></div><div><h3 className="text-xs font-mono font-bold text-zinc-500 uppercase tracking-widest mb-2 flex items-center gap-2"><Briefcase size={14}/> Invoice URL (Stripe)</h3><input className="w-full bg-transparent border-b border-zinc-800 py-2 text-zinc-300 text-sm font-light outline-none focus:border-white" value={selectedClient.paymentUrl || ""} onChange={(e) => updateClientField("paymentUrl", e.target.value)} placeholder="https://buy.stripe.com/..." /></div></div><h3 className="text-xs font-mono font-bold text-zinc-500 uppercase tracking-widest mb-4 flex items-center gap-2"><Paperclip size={14}/> Deliverables</h3><div className="flex-grow space-y-2 mb-4">{selectedClient.assets?.map((asset: any, i: number) => (<div key={i} className="flex justify-between items-center bg-white/5 p-3 rounded-lg border border-white/5 hover:border-white/20 transition-colors group"><div className="flex items-center gap-3"><div className={`w-8 h-8 rounded flex items-center justify-center ${asset.uploader === 'admin' ? 'bg-white text-black' : 'bg-zinc-800 text-zinc-400'}`}><Paperclip size={14} /></div><div><p className="text-sm font-bold text-zinc-300 group-hover:text-white">{asset.name}</p><p className="text-[10px] text-zinc-600 uppercase tracking-widest">{asset.uploader}</p></div></div><a href={asset.url} target="_blank" className="text-zinc-500 hover:text-white transition-colors"><ExternalLink size={14} /></a></div>))}</div><div className="border border-dashed border-zinc-800 rounded-xl p-4 text-center hover:border-white/30 transition-all relative cursor-pointer"><input type="file" onChange={uploadAsset} className="absolute inset-0 opacity-0 cursor-pointer" /><div className="flex flex-col items-center gap-2 text-zinc-500"><Paperclip size={18} /><span className="text-[10px] uppercase font-bold tracking-widest">{status === 'saving' ? "Uploading..." : "Upload Deliverable"}</span></div></div></div></div>
                    </div>
                )}
            </div>
        )}
        {tab === "inbox" && <div className="max-w-4xl mx-auto space-y-4">{leads.map(l => <div key={l.id} className="bg-black/20 p-6 rounded-xl border border-white/5"><h3 className="font-bold text-lg">{l.name}</h3><p className="text-zinc-500 font-mono text-xs uppercase tracking-widest mb-4">{l.email}</p><p className="text-zinc-300 font-light leading-relaxed mb-6">"{l.message}"</p><div className="flex justify-end gap-4"><button onClick={() => handleDelete("leads", l.id)} className="text-red-500/50 hover:text-red-500 text-xs font-bold uppercase tracking-widest transition-colors">Archive</button></div></div>)}</div>}
        {tab === "blog" && (
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="space-y-4">{blogPosts.map(p => <div key={p.id} className="bg-black/20 p-6 rounded-xl border border-white/5 flex justify-between items-center"><h3 className="font-bold">{p.title}</h3><button onClick={() => handleDelete("posts", p.id)}><Trash2 size={16} className="text-zinc-600 hover:text-red-500 transition-colors"/></button></div>)}</div>
                </div>
        )}  
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            {leads.map((l) => (
            <div key={l.id} className="bg-black/20 p-8 rounded-2xl border border-white/5 group hover:border-white/10 transition-all">
                <div className="flex justify-between items-start mb-6">
                <div>
                    <h3 className="text-2xl font-light text-white">{l.name}</h3>
                    <p className="text-zinc-500 font-mono text-xs uppercase tracking-widest mt-1">{l.email}</p>
                </div>
                ...
                <button 
                    ...
                    className="text-emerald-500 hover:text-white text-[10px] font-bold uppercase tracking-widest transition-colors flex items-center gap-2"
                >
        ...
        <AnimatePresence>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-zinc-950 border border-white/10 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                    ...
                    {newItem.type === "client" && (
                        <>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Client Email</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.email} onChange={(e) => setNewItem({...newItem, email: e.target.value})} /></div>
                                <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Project Name</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.title} onChange={(e) => setNewItem({...newItem, title: e.target.value})} /></div>
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Current Status</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.status} onChange={(e) => setNewItem({...newItem, status: e.target.value})} /></div>
                                <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Due Date</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.dueDate} onChange={(e) => setNewItem({...newItem, dueDate: e.target.value})} /></div>
                            </div>
                        </>
                    )}
                    ...
                    {newItem.type !== "client" && (
                        <>
                            ...
                            {newItem.type === "project" && (
                                <div className="grid grid-cols-3 gap-4 p-4 bg-zinc-900/50 rounded-xl border border-white/5">
                                    <label className={`cursor-pointer border p-4 rounded-lg flex flex-col items-center gap-2 hover:bg-white/5 transition-all ${newItem.orientation === 'square' ? 'border-white bg-white/5' : 'border-zinc-800 text-zinc-500'}`} onClick={() => setNewItem({...newItem, orientation: 'square'})}>
                                        <Square size={24} />
                                        <span className="text-[10px] uppercase font-bold tracking-widest">Square</span>
                                    </label>
                                    <label className={`cursor-pointer border p-4 rounded-lg flex flex-col items-center gap-2 hover:bg-white/5 transition-all ${newItem.orientation === 'landscape' ? 'border-white bg-white/5' : 'border-zinc-800 text-zinc-500'}`} onClick={() => setNewItem({...newItem, orientation: 'landscape'})}>
                                        <Monitor size={24} />
                                        <span className="text-[10px] uppercase font-bold tracking-widest">Landscape</span>
                                    </label>
                                    <label className={`cursor-pointer border p-4 rounded-lg flex flex-col items-center gap-2 hover:bg-white/5 transition-all ${newItem.orientation === 'portrait' ? 'border-white bg-white/5' : 'border-zinc-800 text-zinc-500'}`} onClick={() => setNewItem({...newItem, orientation: 'portrait'})}>
                                        <Smartphone size={24} />
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Title</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.title} onChange={(e) => setNewItem({...newItem, title: e.target.value})} /></div>
                                <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Category</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.category} onChange={(e) => setNewItem({...newItem, category: e.target.value})} /></div>
                            </div>

                            {newItem.type === "project" && (
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Order Priority</label><input type="number" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.order} onChange={(e) => setNewItem({...newItem, order: Number(e.target.value)})} /></div>
                                    <div className="space-y-2"><label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Live URL</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none" value={newItem.url} onChange={(e) => setNewItem({...newItem, url: e.target.value})} /></div>
                                </div>
                            )}

                            <div className="space-y-2">
                                <label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Cover Visual</label>
                                <div className="border border-dashed border-zinc-800 rounded-xl p-6 text-center hover:border-zinc-600 relative bg-zinc-900/50">
                                    ...
                                    <div className="flex flex-col items-center gap-2 text-zinc-500"><ImageIcon size={24} /><span className="text-xs uppercase tracking-widest">Select Image</span></div>
                                </div>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">{newItem.type === "post" ? "Body Content" : "Description"}</label>
                                <textarea className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-white outline-none h-32 resize-none" value={newItem.description} onChange={(e) => setNewItem({...newItem, description: e.target.value})} />
                            </div>
                        </>
                    )}
                ...
                <div className="p-6 border-t border-white/5 bg-white/5 flex justify-end gap-3">
                    <button onClick={() => setIsModalOpen(false)} className="px-6 py-3 text-zinc-500 hover:text-white text-xs font-bold tracking-widest uppercase">Cancel</button>
